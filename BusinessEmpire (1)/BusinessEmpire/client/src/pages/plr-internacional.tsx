...
export default function PlrInternacional() {
  ...
  const { data: transactions, isLoading } = useQuery<Transaction[]>({
    queryKey: ["/api/transactions", "plr_internacional"],
    queryFn: async () => {
      const res = await fetch("/api/transactions?category=plr_internacional", {
        credentials: "include",
      });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/transactions", "plr_internacional"] });
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold">PLR Internacional</h1>
